...
def build_func_data(types, f):
    return [_fdata_map.get(t, '%s') % (f,) for t in types]
...
def TD(types, f=None, astype=None, in_=None, out=None, cfunc_alias=None,
    ...
    for t, fd, i, o in zip(types, func_data, in_, out):
        ...
        simdt = [k for k, v in simd if t in v] if simd is not None else []
        # [(dispatch file name without extension '.dispatch.c*', list of types)]
        dispt = ([k for k, v in dispatch if t in v]+[None])[0] if dispatch else None
...
def make_ufuncs(funcdict):
    ...
    for name in names:
        ...
        sig = "NULL" if uf.signature is None else '"{}"'.format(uf.signature)
        fmt = textwrap.dedent("""\
